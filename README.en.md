...
- **Mini apps** — Seven one-task sandboxed tools; each card on the home wall names the capability it needs and offers **Configure** when something is missing: **Background remover** (local ONNX engine, nothing uploaded, ~1s, recolor plus erase/restore brushing), **ID photo** (local portrait cutout resized to 20 specs — 1-inch, 2-inch, visa and more — with background swap and A4 print sheets), **Mosaic** (paint-to-redact with batch export, entirely on-device), **Portrait** (style presets for professional headshots and character avatars), **Meeting notes** (import or record audio → transcribe → agenda / conclusions / action items), **Copywriter** (multiple titles, selling points or scripts by purpose and tone), and **Notes** (calendar and tag views, image attachments, bodies stored in the local database, optionally exposed to the agent read-only). Each mini app is one self-contained HTML page running in a sandboxed iframe **without** `allow-same-origin`, and the host grants capabilities one by one.
...
- [x] Mini apps: 7 sandboxed single-page tools with per-capability gating
